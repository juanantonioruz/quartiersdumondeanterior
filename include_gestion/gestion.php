...
function gestionar(){
	...
	if($_GET[id_dato_zona_gestion]) $_SESSION[web_actual]=$_GET[id_dato_zona_gestion];
	$this->web_actual=$_SESSION[web_actual];
	#echo "<h1>EE".$this->web_actual."</h1>";
    $this->check_registro();
    $this->select_pack();
...
if($_GET[id_seccion] && $this->archivo=='index.php'){
 ...
 $this->last_id_seccion=$_SESSION[last_id_seccion];
...
if($this->depurar_gestion_index) echo "<hr>".$this->idioma_traduccion."<hr>";
}//function
...
foreach($_SESSION["sqls"][$this->pack_name] as $clave_pack=>$valor_pack) 
    ...
    $info.="\n\n";
#foreach($_SESSION[$this->id_seccion] as $clave_ahora) $info.= "\n<span class=calida>".$clave_ahora ."</span><span class=azul></span<br>";
...
$this->navega_sup_info="\n <a href='../c_99/index.php' class='menu_sup_nav'><img src='../images/licq.png' width=70> Inicio </a> &gt; &gt; ".$oyo."<a href='index.php?id_seccion=".$this->id_seccion."' class='menu_sup_nav'>".$this->img_gestion." ".$this->seccion_actual."</a>";
if($oyo_complemento) $this->navega_sup_info.="  <hr color='white'><br><br><div class='tam_h3_white' align='center'>&gt; &gt; $oyo_complemento</div> \n\n"; #.$_SESSION["sqls"][$this->pack_name][$this->seccion]["info_zona_00"].
}//f_ navega_sup
...
switch ($tipo_campo){
	...
	case "fecha":
			...
			$resultado.="<b >$campo_trad</b><br> ".$this->de_html($valor)." <br>\n\n";
			break;
	...
	case "img":
			if($argumentos){
			$arra_argumentos=explode( "&", $argumentos);
			$min_width=$arra_argumentos[0];
			$min_height=$arra_argumentos[1];
			$max_width=$arra_argumentos[2];
			$max_height=$arra_argumentos[3];
			$path_destino=$arra_argumentos[4];
			}else{
			$min_width=100;
			$min_height=100;
			$max_width=500;
			...
			$resultado.="<b>$campo_trad</b><br><a name='$id_campo'>".$this->upload->archivo_upload("$id_campo", '50000', $min_width, $min_height, $max_width, $max_height, $path_destino,  $this->traduce, "img"). "<br>";
			break;
	case "doc":
			$resultado.="<b>$campo_trad</b><br><a name='$id_campo'>".$this->upload->archivo_upload("$id_campo", '50000', 600, 600, "","","", $this->traduce, "doc"). "<br>";
			break;

	case "mapa":
			if($_POST[$id_campo]==null OR $_POST[$id_campo]=='')$valor=$this->{'select_'.$id_campo};
			else $valor=$_POST[$id_campo];
			if(!$valor)$valor='0x0x0';
			$argumentos_valores=explode("x", $valor);
			if($argumentos_valores[0]<>0) $mini="<a href='#' onclick=\"javascript:window.open('../mapa/".$argumentos_valores[0]."','pop','toolbar=no, location=no, status=no, width=400, height=400, left=5, top=5, scrollbars=yes, resizable=yes')\" class='seleccion'><img src='../mapa/".$argumentos_valores[0]."' width=150 height=150 border=1></a><br>"; 
			$c_x=$argumentos_valores[1];
			$c_y=$argumentos_valores[2];
			$inf_visible="<b>x</b>=$c_x <b>y</b>=$c_y <br>";
			$resultado.="<b>$campo_trad</b><br>$inf_visible
			<input type='hidden' name='$id_campo' value='$valor'>
			$mini
			<a name='$id_campo'><a href='#' onclick=\"javascript:window.open('../mapa/map_float.php?x=$c_x&y=$c_y','pop','toolbar=yes, location=no, status=yes, width=747, height=486, left=5, top=5, scrollbars=yes, resizable=yes')\" class='seleccion'>seleccionar coordenadas en el plano</a><br>\n";
			#$resultado.="<b>$campo_trad</b><br><a name='$id_campo'>".$this->upload->archivo_upload("$id_campo", '50000', 600, 600, "","","", $this->traduce, "doc"). "<br>";
			break;
	...
	case "textarea":
            if($tipo_carga_datos=='consulta'){
				if($_POST[$id_campo]==null OR $_POST[$id_campo]=='')$valor=$this->{'select_'.$id_campo};
				else $valor=str_replace("\'","'", $_POST[$id_campo]);
				}elseif($tipo_carga_datos=='directo'){
				$valor=$argumentos;
				}
				if($this->archivo<>'del.php')
				...
				$resultado="<b>$campo_trad</b><br>$valor<br>$salto$salto\n\n";
				break;
	...
	case "textarea_p":
			...
			$valor=$argumentos;
				...
				if($this->archivo<>'del.php')
				$resultado="<b>$campo_trad</b> <textarea name='".$id_campo."' cols=40 rows=3 class='forms'>$valor</textarea><br>$salto$salto\n\n";
				else
				$resultado="<b>$campo_trad</b><br>$valor<br>$salto$salto\n\n";
				break;
	...
	case "radio":
			...
			$resultado.="<b >$campo_trad</b><br> ". $found." \n<br>\n\n";
			}//IF
			break;   
