...
class upload extends global_interface{
    ...
    var $archivo='';
    var $timestamp='';
    var $espacio_maximo_directorio=1000000; #kb
	var $miniatura_width=100;
	var $miniatura_height=100;
    
function upload(){
if($_POST[path_destino]){
 $this->path_definitivo=$_POST[path_destino];
$this->miniatura_width=$_POST[min_width];
$this->miniatura_height=$_POST[min_height];
 }

$this->graba_variables_obj_ppal(func_get_args());
/*
mira la información del archivo que llama al upload para sacar la seccion y el archivo
y crea 2 o 4 sesiones
*/
$this->seccion="c_".$_SESSION[last_id_seccion];
...
elseif($this->archivo=='win_upload.php') $this->win_upload();
elseif($this->archivo=='win_upload_del.php') $this->del_archivo();
}//function upload


function archivo_upload($nombre_archivo, $tamanyo_max, $min_width, $min_height, $max_width, $max_height, $path_destino,$array_idiomas=null, $img_o_doc="img"){
#echo "<b>".$argumentos."</b>";
if($path_destino)
$this->path_definitivo=$path_destino;
else $this->path_definitivo="../../imgs/";
if(!$this->j_script_echo) $info_up.=$this->j_script_upload();
if($img_o_doc=="img"){ $img_mostrar="camera.png"; $colocar=$this->conecto_central->traduccion[gest_upload_colocar_imagen];}else{ $img_mostrar="files.png"; $colocar=$this->conecto_central->traduccion[gest_upload_colocar_archivo];}
...
if($this->depurar_upload) $this->depurar_mensaje("<b>sesion en llamada a imagen:</b><bR>".$nombre_archivo."=".$ses_nombre_archivo);
	if(!$ses_nombre_archivo){
		$info_up.= "\n\n<a href=\"javascript:uploading_form('$nombre_archivo','$min_width','$min_height','$max_width','$max_height','$path_destino','$tamanyo_max');\"><img src='../images/$img_mostrar' width=80></a> <br>\n\n";
	}else{
		$this->select_tipo_archivo($ses_nombre_archivo);
		if($this->archivo<>'del.php'){
		if($this->seccion<>'c_11'){
		$info_up.= "<a href=\"javascript:uploading_form('$nombre_archivo','$min_width','$min_height','$max_width','$max_height','$path_destino','$tamanyo_max');\"><img src='../images/edit.png'></a> ";
		$info_up.= "<a href=\"javascript:eliminar('$ses_nombre_archivo','$nombre_archivo','$path_destino');\"><img src='../images/editdelete.png'></a> <br>";
		}else{
		if(@in_array(substr($nombre_archivo, -3), $array_idiomas))
		$info_up.= "<a href=\"javascript:uploading_form('$nombre_archivo','$min_width','$min_height','$max_width','$max_height','$path_destino','$tamanyo_max');\"><img src='../images/edit.png'></a> ";
		$info_up.= "<a href='".$this->path_definitivo.$ses_nombre_archivo."' target='_blank'>download</a><br> ";#".$this->conecto_central->traduccion[gest_upload_descargar_archivo]."
		}//c_05
		}//del.php
		 $info_up.= "\n<a name='$nombre_archivo'></a><img src='".$this->v_imagen."' border='1' ><input type='hidden' name='$nombre_archivo' value='".str_replace('".$this->path_definitivo."','', $ses_nombre_archivo)."'><br>\n";
	 }//if
return $info_up;
}//function
...
$path_destino=$_GET['path_destino'];
$tamanyo_max=$_GET['tamanyo_max'];
/*
foreach($_SESSION["c_29"] as $clave)
foreach($clave as $c=>$v) $form.=" ___ $c $v<hr>";
*/
$form.="
...
if(window.document.forms['upload_form']['$nombre_archivo'].value){
   ...
   window.document.forms['upload_form'].submit();
    ...
    alert('".$this->conecto_central->traduccion[gest_upload_sel_archivo]."');
}// if
}
-->
</script>
<form enctype='multipart/form-data' action='win_upload.php' target='pop'  name='upload_form' method='post'>\n";
if($max_height) $form.= "<input type='hidden' name='max_height' value='$max_height'>\n";
if($max_width) $form.= "<input type='hidden' name='max_width' value='$max_width'>\n";
if($min_height) $form.= "<input type='hidden' name='min_height' value='$min_height'>\n";
if($min_width) $form.= "<input type='hidden' name='min_width' value='$min_width'>\n";
if($path_destino) $form.= "<input type='hidden' name='path_destino' value='$path_destino'>\n";
$form.= "<input name='$nombre_archivo' type='file' size=30>\n";
if($tamanyo_max) $form.= "<input type='hidden' name='MAX_FILE_SIZE' value=$tamanyo_max>\n";
$form.= "<input name='nombre_archivo' value='$nombre_archivo' type='hidden' >\n";
$form.= "<input name='valor_archivo' value='' type='hidden' >\n";
$form.= "<br><input type='button' onclick='javascript:uploading();' value='".$this->conecto_central->traduccion[gest_upload_enviar]."'><input type='button' onclick='javascript:window.self.close();' value='".$this->conecto_central->traduccion[gest_upload_cerrar_ventana]."'></form><br>\n";
$this->documento_gestion->salida($form, true, true);
}// f_ upload_form()
...
function win_upload(){
	if($_POST['enviar'] or $_POST['borrar']){
		...
		if(!$this->temporal_copy()) $this->documento_gestion->salida($this->conecto_central->traduccion[gest_upload_intentar_otra_vez]."<br><input type='button' onclick='javascript:window.self.close();' value='".$this->conecto_central->traduccion[gest_upload_cerrar_ventana]."' >");
	}//if
}// function win_upload
...
$path_destino=$_POST['path_destino'];
...
unlink($this->path_temporal.$this->archivo_def_up.$this->extension_temporal);
    $this->documento_gestion->salida("Su capacidad actual ($tamanyo kb) ha sobrepasado el limite de capacidad de su disco (".$this->espacio_maximo_directorio." kb). pongase en contacto con <a href='mailto:info@mediaymedia.com'>info@mediaymedia.com</a> para solucionar el problema<br><br><input type='button'  onclick='javascript:window.self.close();' value='cerrar_ventana' name='cerrar_ventana'></body>");
die();
}
    $this->documento_gestion->salida('');
   ...
   if($img_width) echo "$img_width <br>
    ...
    $img_medidas<br>
    $nombre_imagen<br>";
    $form.= "<form action='' method='post' name='f_aprobacion'  id='f_aprobacion' >";
if($max_height) $form.= "<input type='hidden' name='max_height' value='$max_height'>\n";
if($max_width) $form.= "<input type='hidden' name='max_width' value='$max_width'>\n";
if($min_height) $form.= "<input type='hidden' name='min_height' value='$min_height'>\n";
if($min_width) $form.= "<input type='hidden' name='min_width' value='$min_width'>\n";
if($path_destino) $form.= "<input type='hidden' name='path_destino' value='$path_destino'>\n";
	...
	$form.="<input type='hidden' name='archivo_clave' value='".$this->archivo_upload."'>
...
if(!$_SESSION[$this->seccion_referer][$this->archivo_referer][$this->archivo_upload]){
  $form.="<input type='submit' value='".$this->conecto_central->traduccion[gest_ok]."' name='enviar'>";
   $form.="<input type='submit' value='".$this->conecto_central->traduccion[gest_no_ok]."' name='borrar'>";
   }else{
   $form.="<br><input type='submit' value='".$this->conecto_central->traduccion[gest_upload_modificar]."' name='enviar'>
<input type='submit'  value='".$this->conecto_central->traduccion[gest_upload_no_modificar]."' name='borrar'>";
   }//if
    $form.= "</form>
    ";
	echo $form;
...
$this->archivo_def_up= str_replace($this->extension_temporal, ".jpg", $this->archivo_def_up);
 ...
 $min_ancho=$this->miniatura_width;
$min_alto=$this->miniatura_height;
...
#echo substr($ses_nombre_archivo,-3)."<br>";
   ...
   case "jpg":
   $v_imagen=$this->path_definitivo.$ses_nombre_archivo;
   break;
   case "peg":
   $v_imagen=$this->path_definitivo.$ses_nombre_archivo;
   break;
...
$info_script.= "\n\n<SCRIPT language=JavaScript>
<!--
function uploading_form(nombre_archivo, min_width, min_height, max_width, max_height, path_destino,tamanyo_max){
window.open('../c_upload/win_upload_form.php?nombre_archivo='+nombre_archivo+'&min_width='+min_width+'&min_height='+min_height+'&max_width='+max_width+'&max_height='+max_height+'&path_destino='+path_destino+'&tamanyo_max='+tamanyo_max,'pop','toolbar=no, location=no, status=yes, width=400, height=400, left=0, top=0, scrollbars=yes, resizable=yes');
}
function eliminar(img_valor, img_clave, path_destino){
window.open('../c_upload/win_upload_del.php?img='+img_valor+'&clave='+img_clave+'&path_destino='+path_destino+'&path=../".$this->seccion."/".$this->archivo."','pop','toolbar=yes, location=no, status=yes, width=400, height=400, left=0, top=0, scrollbars=yes, resizable=yes');
}
//-->
</SCRIPT>\n\n";
...
if($this->depurar_upload) $this->depurar_mensaje("<b>Aplicar sesiones:</b><bR>".$this->seccion." ".$t_archivo."=".$_SESSION[$this->seccion][$this->archivo][$t_archivo]);
}//function
...
if($_POST['enviar']=='ok'){
    ...
    #echo "<hr>".$this->seccion_referer." ".$this->archivo_referer." ".$_POST['archivo_clave']."<hr>".str_replace('nn', 'mm', $_POST['archivo_valor'])."<hr>";
	}else{
...
$borrar_anterior=$_SESSION[$this->seccion_referer][$this->archivo_referer][$_POST['archivo_clave']];
    @unlink($this->path_definitivo.$borrar_anterior);
    @unlink($this->path_temporal.str_replace(".jpg", ".gif", $_POST['archivo_valor']));
    @unlink($this->path_temporal.str_replace(".jpg", ".png", $_POST['archivo_valor']));
    @unlink($this->path_definitivo.str_replace('mm', 'zz', $borrar_anterior));
    @unlink($this->path_definitivo.str_replace('nn', 'mm', $borrar_anterior));
    #die();
    if(@copy($this->path_temporal.str_replace('nn', 'mm', $_POST['archivo_valor']), $this->path_definitivo.str_replace('nn', 'mm', $_POST['archivo_valor'])))
        copy($this->path_temporal.str_replace('nn', 'zz', $_POST['archivo_valor']), $this->path_definitivo.str_replace('nn', 'zz', $_POST['archivo_valor']));
    else
...
window.opener.document.forms[0].action='../c_90/".$this->archivo_referer.$sesid."'+window.opener.document.forms[0].action+'';
//alert(window.opener.document.forms[0].action);
//alert('".$this->archivo_referer."');
}
...
if($_POST[PHPSESSID])$sesid="?PHPSESSID=".$_POST[PHPSESSID];
$path="../c_90/".$this->archivo_referer.$sesid."#".$_POST['img_clave'];
echo "<SCRIPT language=JavaScript>
<!--
if('".$this->seccion_referer."'!='c_05'){
...
if($_GET){
$this->path_definitivo=$_GET['path_destino'];
$this->select_tipo_archivo($_GET['img']);
$prev=$this->v_imagen;
$this->documento_gestion->salida("<img src='$prev'>
    <form action='' method='post' name='f_aprobacion'  id='f_aprobacion' >
    <input type='hidden' name='path' value='".$_GET['path']."'>
    <input type='hidden' name='path_destino' value='".$_GET['path_destino']."'>
    <input type='hidden' name='img_valor' value='".$_GET['img']."'>
    <input type='hidden' name='img_clave' value='".$_GET['clave']."'>
	<br><br><input type='submit' value='".$this->conecto_central->traduccion[gest_upload_borrar]."' name='borrar'>
	<input type='button' onclick='javascript:window.self.close();' value='".$this->conecto_central->traduccion[gest_upload_no_borrar]."' name='no_borrar'></form>");
    }elseif(!$_GET){
$this->documento_gestion->salida($this->conecto_central->traduccion[gest_upload_error_borrar]."<br><input type='button' onclick='javascript:window.self.close();' value='".$this->conecto_central->traduccion[gest_upload_intentar_nuevamente]."' >");
    }
}//funtion del_archivo
