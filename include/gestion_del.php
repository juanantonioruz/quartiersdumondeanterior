...
$this->sql_mod_update_tabla[]=$this->conecto_central->Record[dato_tabla];
...
$this->conecto_central->query("select id_dato_tabla, dato_campo, dato_campo_join, dato_campo_tipo, tabla_multiple, tabla_datos_central 
from  dato_tabla 
left join dato_campo on (dato_campo_id_tabla=id_dato_tabla)
where dato_tabla='$valor' and (dato_campo_tipo='img'  or dato_campo_tipo='doc'  or dato_campo_tipo='referencia')");
$img_delete='';
while($this->conecto_central->next_record()){
if($this->conecto_central->Record[tabla_datos_central]) $datos="datos_central"; else $datos="datos";
#echo "<h3> ".$this->conecto_central->Record[dato_campo]." </h3>";
...
#echo "<h1> $valor</h1>";
    if($img_delete){
        if($jjj){ // es decir que es la tabla multiple
            foreach($ids_valores_multiple as $vv){
                $this->$datos->query("select ".implode(', ', $img_delete)." from $valor where ".$ref_delete."=".$vv);
            ...
            $this->$datos->query("select ".implode(', ', $img_delete)." from $valor where ".$ref_delete."=".$this->id_seccion);
...
if(!in_array($imagen_del, $arra_def_del_images)) $arra_def_del_images[]=$imagen_del;
if(!in_array(str_replace("mm", "zz", $imagen_del), $arra_def_del_images))  $arra_def_del_images[]=str_replace("mm", "zz", $imagen_del);
}//foreach
}//if
}
}//for
if(is_array($arra_def_del_images))
foreach($arra_def_del_images as $imagen_del){
@unlink("../../imgs/". $imagen_del);
#echo "../../imgs/".$imagen_del."<br>";
}
/*
...
$this-> datos_central->query("delete from registro_00 where id_registro_00=".$_POST[id_registro_00]);
$this-> datos_central->query("delete from registro_01 where registro_01_id_registro_00=".$_POST[id_registro_00]);
}
...
$info_del.="<tr><td colspan=2 bgcolor='#FFFFFF'><input type='hidden' value='' name='actualizar'><input type='button' name='modificar' value='$boton_eliminar' onclick=\"javascript:window.self.document.forms[0]['actualizar'].value='go';window.self.document.forms[0].submit();\"></td></tr></table></form>";
